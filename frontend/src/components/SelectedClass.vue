<template>
  <div id="SelectedClass" v-loading="loading">
    <el-row>
        ...
        <el-col :span="6">
          <el-form-item>
            <el-button type="primary" icon="el-icon-search" style="float:left;margin-left: 0px"
                       v-on:click="get_selected_class_by_condition">查询
            </el-button>
          </el-form-item>
        </el-col>
      </el-form>
    ...
    <el-table :data="page_classes" ref="class"
      ...
      <el-table-column prop="option" label="操作" min-width="100" align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.operate!=1&&scope.row.status==1" size="medium" type="danger" plain
                     @click="out_class(scope.row.cid_id)">撤课
          </el-button>
          <p v-if="scope.row.operate==1">申请中</p>
          <p v-if="scope.row.status==0">已结课</p>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  export default {
    ...
    methods: {
      ...
      get_selected_class_by_condition() {
        let that = this
        ...
        that.axios({
          method: 'post',
          url: '/api/get_selected_class_by_condition/',
          data: param
        })
          .then(function (res) {
            if (res.data.code === 1) {
