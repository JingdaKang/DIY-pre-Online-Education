    ...
    <el-table-column align="right">
      <template slot-scope="scope">
        ...
        <el-button
          size="mini"
          type="danger"
          @click="handleFollow(scope.$index, scope.row)">{{scope.row.follow_user_if_following === 1?"已关注":"关注对方"}}</el-button>
      </template>
    </el-table-column>
...
<script>
  export default {
    ...
    methods:{
      getInfo(){
        this.axios.get('/forum/userFollower',{
          params: {
            user: this.user,
          }
        }).then((response)=>{
